...
def compute_meta_new(
    token_lora_tensor: torch.Tensor, seq_lens: List[int]
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int, int, bool]:
    """
    Get the information required for the sgmv kernel. With the  features:
    1. If consecutive requests in the batch use the same LoRA, this function
    ...
    # lora_indices_tensor, seq_length_tensor = torch.unique_consecutive(
    #     token_lora_tensor, return_counts=True)
    lora_indices_tensor = token_lora_tensor.clone()
    seq_length_tensor = torch.tensor(seq_lens, device="cuda", dtype=torch.long)
    cum_result = torch.cumsum(seq_length_tensor, dim=0)
    b_seq_start_tensor = torch.zeros_like(seq_length_tensor)
    b_seq_start_tensor[1:].copy_(cum_result[:-1])
    max_length = seq_length_tensor.max().item()
...
def convert_mapping_new(
    ...
    # lora index for linear-based layer
    lora_index_seqs = list(mapping.lora_index_reqs).copy()
    base_indices_lst = []
    for i in range(len(lora_index_seqs)):
        lora_idx = (lora_index_to_id.index(lora_index_seqs[i])
                    if lora_index_seqs[i] > 0 else -1)
        base_indices_lst.append(lora_idx)
    base_indices = torch.tensor(base_indices_lst,
                                device="cuda",
                                dtype=torch.long)
...
class PunicaWrapper:
    ...
    def update_metadata(
        ...
        if mapping.is_prefill:
            ...
            self._update_prefill_metada_new(self.token_lora_indices,
                                            mapping.seq_lens)
    ...
    def _update_prefill_metada_new(self, token_lora_tensor: torch.Tensor,
        ...
        (b_seq_start_tensor, seq_length_tensor, lora_indices_tensor,
         ...
         no_lora) = compute_meta_new(token_lora_tensor, seq_lens)
