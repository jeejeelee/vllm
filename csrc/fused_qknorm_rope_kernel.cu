...
__device__ inline float compute_freq_yarn(float base, int head_dim,
                                          int half_dim, float factor, float low,
                                          float high) {
  ...
  if constexpr (interleave) {
    ...
    if (in_rotary) {
      for (int i = 0; i < numElemsPerThread; i++) {
        ...
        float freq =
            compute_freq_yarn(base, rotary_dim, half_dim, factor, low, high);
    ...
    if (in_rotary) {
      ...
      unsigned int active_mask =
          (1u << rotary_lanes) - 1;  // 低16位为1: 0x0000FFFF
      for (int i = 0; i < numElemsPerThread; i++) {
        elements2[i] =
...
void launchFusedQKNormRope(void* qkv, int const num_tokens,
                           ...
                           void const* k_weight, float const base,
    ...
    case 64:
      DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
        fusedQKNormRopeKernel<64, INTERLEAVE><<<gridDim, blockDim, 0, stream>>>(
            ...
            reinterpret_cast<__nv_bfloat16 const*>(k_weight), base,
            position_ids, num_tokens, factor, low, high, attention_factor,
            rotary_dim);
      });
      break;
    case 128:
...
// This operator applies RMS normalization and RoPE to Q and K tensors in a
// single CUDA kernel. The OP performs operations in-place on the input qkv
// tensor.
void fused_qk_norm_rope(
    ...
    // parameters for yarn
    double factor,  // factor in rope_scaling in config.json. When it is
                    // not 1.0, it means the model is using yarn.
  ...
  TORCH_CHECK(rotary_dim % (head_dim / 32) == 0,
              "rotary_dim must be divisible by numElemsPerThread");
  TORCH_CHECK((rotary_dim / (head_dim / 32)) % 2 == 0,
              "rotary_lanes must be even for neox style");
  ...
  tensorrt_llm::kernels::launchFusedQKNormRope(
      ...
      static_cast<float>(factor), static_cast<float>(low),
      static_cast<float>(high), static_cast<float>(attention_factor),
      static_cast<int>(rotary_dim), stream);
}